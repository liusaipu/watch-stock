...
常用命令
--------
  watch.cmd                    简化模式，15秒刷新（推荐）
  ./watch 5 --index on         macOS 下 5 秒刷新并显示指数方块
  watch.cmd 30                 简化模式，30秒刷新
...
------
  ...
  B / F9       切换为 npm 构建日志外观 / 正常行情
  S            切换简化模式（灰白英文）/ 详情模式（中文红绿）
  ↑ / ↓        移动光标
  →            进入盘口详情（五档买卖、逐笔成交）
  ← / Esc      盘口页返回列表
...
--------
  --index on|off     是否显示指数方块，默认 off
                     （简化模式会固定显示上证指数、深证成指）
  --stealth on|off   简化模式（灰白文字、低调窗口标题），默认 on
...
注意事项
--------
- 简化模式默认灰白英文显示，窗口标题为 System
- 按 B 键立即切换为 npm 构建日志外观，再按恢复
